...
const calcDisplaySummary = function (acc) {
  ...
  const inout = acc.movements
    .filter(mov => mov < 0)
    .reduce((acc, mov) => acc + mov, 0);
  labelSumOut.textContent = `${Math.abs(inout)}€`;
...
// SET (value === key)
const currenciesUnique = new Set(['USD', 'USDT', 'NEAR', 'LONA', 'EUR']);
...
const JuliaCopy = Julia.splice(1, 3);
console.log(JuliaCopy);

const JuliaKate = [...JuliaCopy, ...Kate];

JuliaKate.forEach(function (dog, i) {
  const typeDog = dog > 3 ? 'an adult' : 'an puppy';
  console.log(`Dog number ${i + 1} is ${typeDog}, and is ${dog} years old`);
}); */
